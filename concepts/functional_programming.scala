...
// (good modularity)
// modularization:
//   ways of decomposition (divide) and glues (combine)
//   divide into subproblems and combine solutions
//   the power of divide depends directly on the power of glues
// function programming provides two new kinds of glues (combine):
//   it has smaller, simpler, yet powerful modularizations
// 1) higher-order functions
//    ex. map and reduce function (generalized and parameterized functions that return a class of functions)
// 2) lazy evaluation (in every function call)
//    Streams: lazy list
//    Generators: for/yield construct
//      coroutines that run in strict synchronization
//      a) generator      : produces a (maybe infinite) number of results
//      b) selector/filter: chooses appropriate answer generated by generator
//    performance advantage:
//      f is only started (and run to deliver the output) once g tries to read some input
//      no extra memory is required to store the answer
//
// structured programming: 
// 1) single-entry and single-exit (no GOTO statement)
// 2) complicated scope rules
// 3) provision for separate compilation
//    ex. instiantiation of objects
// 4) NOT pure function: HAVE side effects
//    a function may return different values when called with same parameters multiple times
//    ex. function that returns the current temperature, it reads a temperature sensor (mutable resource) 
//    ex. modify passed-in variables, write to external dependent resources (ex. a file or database I/O)
//
