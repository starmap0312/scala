...
//    the following two are run in strict synchronization
//      ex. f is only started (and run to deliver the output) once g tries to read some input
//    a) generator: may produce a infinite number of results (i.e. no termination in loop condition)
//    b) selector:  choose appropriate answer generated by generator
//    advantage: no extra memory is required to store the answer
//    example  : streams (lazy lists)
//
